...
class CacheAccessor {
    ...
    private $available_keys=array('odds');
    ...
    public function zunionstore($group,$destination,$keys) {
        if(in_array($group, $this->available_keys)) {
            $exist_keys=array();
            //$exist_keys[]=$group.":".$destination;
           foreach($keys as $item) {
                $exist_keys[]=$group.":".$item;
           }

            $this->redis->ZUNIONSTORE($group.":".$destination,$exist_keys);
        } else {
            throw new Exception("{$group} is not support");
        }
    }
